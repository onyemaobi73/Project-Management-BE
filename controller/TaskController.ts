...
export const createTask = async (req: Request, res: Response) => {
    try {
        ...
        res.status(STATUSCODE.CREATE).json({
            message: "task created", data: tasked
        })
    } catch (error: any) {
        return res.status(STATUSCODE.BAD).json({message: "Error creating task", data: error.message})
    }
}
...
export const readTask = async(req: Request, res: Response) =>{
    try {
        ...
        res.status(STATUSCODE.BAD).json({message: "Error reading task", data: error.message})
    }
}
...
export const updateOneTask = async(req: Request, res: Response) =>{
    try {
        const { id } = req.params;
    const tasked = await TaskModel.findByIdAndUpdate(id, { new: true})
...
export const readOneTask = async(req: Request, res: Response) =>{
    ...
    res.status(STATUSCODE.CREATE).json({message: "reading task ", data: tasked})
    } catch (error: any) {
        res.status(STATUSCODE.CREATE).json({message: "Error reading task", data: error.message})
    }
}
...
export const deleteTask = async(req: Request, res: Response) =>{
    try {
        ...
        res.status(STATUSCODE.CREATE).json({message: "task has been deleted", data: tasked})
    } catch (error: any) {
        res.status(STATUSCODE.BAD).json({message: "Error deleting task ", data: error.message})
    }
}